...
const getPasswordStrength = (password) => {
  const { lengthCheck, upperCheck, lowerCheck, specialCheck } =
    validatePassword(password);
  const validCount = [lengthCheck, upperCheck, lowerCheck, specialCheck].filter(
    Boolean
  ).length;

  if (!password) return { label: "Empty", color: "bg-base-300", width: "w-0" };
  if (validCount <= 1)
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ProjectUILogo size={60} onClick={() => navigate("/")} />
        </div>
        ...
        >
          ...
          <span
            onClick={() => navigate("/")} // add your routes
            style={{
              color: "#007bff",
              fontWeight: "600",
              cursor: "pointer",
